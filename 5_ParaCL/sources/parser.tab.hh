...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, name_t*&& v)
        : Base (t)
        , value (std::move (v))
      {}
#else
      basic_symbol (typename Base::kind_type t, const name_t*& v)
        : Base (t)
        , value (v)
      {}
#endif
...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == token::NUMBER || tok == token::FUNC);
      }
#else
...
#endif
#if 201103L <= YY_CPLUSPLUS
      symbol_type (int tok, name_t* v)
        : super_type(token_type (tok), std::move (v))
      {
        YY_ASSERT (tok == token::NAME);
      }
#else
      symbol_type (int tok, const name_t*& v)
        : super_type(token_type (tok), v)
      {
        YY_ASSERT (tok == token::NAME);
      }
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      make_LUNICORN ()
      {
        return symbol_type (token::LUNICORN);
      }
#else
      static
      symbol_type
      ...
      make_RUNICORN ()
      {
        return symbol_type (token::RUNICORN);
      }
#else
      static
      symbol_type
      ...
      make_GRATER ()
      {
        return symbol_type (token::GRATER);
      }
#else
      static
      symbol_type
